"""
HDMC-Net Feeder Utilities

Data augmentation and preprocessing functions for skeleton data.
"""
...
def valid_crop_resize(data_numpy, valid_frame_num, p_interval, window):
    """Crop and resize skeleton sequence to fixed window size."""
    # input: C,T,V,M
    ...
    if valid_size <= 0:
        return np.zeros((C, window, V, M), dtype=np.float32)
...
def temporal_slice(data_numpy, step):
    """Slice skeleton sequence temporally."""
    # input: C,T,V,M
    C, T, V, M = data_numpy.shape
...
def mean_subtractor(data_numpy, mean):
    """Subtract mean from skeleton sequence."""
    # input: C,T,V,M
    if mean == 0:
        return
    C, T, V, M = data_numpy.shape
...
def auto_pading(data_numpy, size, random_pad=False):
    """Auto padding for skeleton sequence."""
    C, T, V, M = data_numpy.shape
    if T < size:
...
def random_choose(data_numpy, size, auto_pad=True):
    """Randomly choose a portion of the skeleton sequence."""
    # input: C,T,V,M
    C, T, V, M = data_numpy.shape
    if T == size:
        return data_numpy
...
def random_move(data_numpy,
    ...
    """Random spatial transformation for data augmentation."""
    # input: C,T,V,M
    C, T, V, M = data_numpy.shape
...
def random_shift(data_numpy):
    """Random temporal shift for data augmentation."""
    C, T, V, M = data_numpy.shape
